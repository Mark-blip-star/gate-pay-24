...
import { ArrowDownToLine, LogOut, UserCircle2 } from 'lucide-react';
import { useEffect, useMemo, useState } from 'react';
import { useForm } from 'react-hook-form';
import { z } from 'zod';
import { useAuth } from '../auth/AuthContext';
import { apiTransactions, apiWithdraw, type Transaction } from '../lib/api';
...
export function DashboardPage() {
  ...
  const {
    ...
    reset,
  } = useForm<WithdrawForm>({ resolver: zodResolver(withdrawSchema), defaultValues: { amount: 25 } });
  ...
  const initials = useMemo(() => (user?.email?.[0] ?? 'U').toUpperCase(), [user?.email]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950 text-white">
        ...
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-white/60">Your transactions dashboard</div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-3 py-2">
              ...
              <div className="hidden sm:block">
                ...
                <div className="text-xs text-white/60">{formatMoney(balance)} available</div>
              </div>
      ...
      <main className="mx-auto max-w-6xl px-4 py-8">
        <div className="mb-6 flex items-end justify-between gap-4">
          <div>
            ...
            <div className="text-sm text-white/60">All activity for your account</div>
          </div>
                ...
                {isLoading ? (
                  ...
                  items.map((t) => (
                    <tr key={t.id} className="hover:bg-white/[0.03]">
                      <td className="px-5 py-4 text-white/70">{new Date(t.createdAt).toLocaleString()}</td>
                      ...
                      <td className="px-5 py-4 font-semibold">
                        <span className={t.type === 'withdraw' ? 'text-amber-200' : 'text-emerald-200'}>
                          {t.type === 'withdraw' ? '-' : '+'}
      ...
      {showWithdraw ? (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4" onClick={() => setShowWithdraw(false)}>
